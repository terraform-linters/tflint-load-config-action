...
const dist_src_Octokit = Octokit.plugin(requestLog, plugin_rest_endpoint_methods_dist_node.legacyRestEndpointMethods, dist_node.paginateRest).defaults(
  {
...
;// CONCATENATED MODULE: ./lib/copy.js



async function copyFile({ owner, repo, srcPath, ref, dstPath, token }) {
    const octokit = new dist_src_Octokit({
        auth: token,
        request: { fetch: (lib_default()) }
    });
    ...
    await external_fs_.promises.writeFile(dstPath, Buffer.from(data.content, 'base64').toString('utf8'));
    return dstPath;
}
